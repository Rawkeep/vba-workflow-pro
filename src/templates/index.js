...
export function loadLibTemplate(type,key){
  ...
  if(S.wH.length){
    ...
    const fixable=unmatched.filter(p=>window._suggestField(p)).length;
...
export function autoGenTemplate(){
  ...
  $('w-tpl').value=tpl;
  window.UP();toast('⚡ Professionelles Template aus '+h.length+' Spalten generiert');
}
...
export function patchRW(origRW){
  return function(){origRW();$('w-insert-data').style.display=S.wI?'':'none';$('w-auto-tpl').style.display=S.wI?'':'none';if(_currentDocType==='etikett')$('w-labels-pdf').style.display='';window.UP()};
}
...
export function EMPDF(){const subj=$('em-subj').value||'E-Mail';const body=`An: ${$('em-to').value}${$('em-cc').value?'\nCC: '+$('em-cc').value:''}\nBetreff: ${subj}\nDatum: ${new Date().toLocaleDateString('de-DE')}\n\n${$('em-body').value}`;window.textToPDF(body,subj.replace(/[^a-zA-Z0-9äöüÄÖÜß ]/g,'_')+'.pdf');toast('PDF ✓')}
export function checkSEM(){const h=S.wH.some(h=>h.toLowerCase().includes('mail'));const btn=$('sem-btn');if(btn)btn.disabled=!S.wI||!h;const info=$('sem-info');if(info)info.textContent=S.wI?(h?S.wD.length+' ✓':'⚠ "Email"?'):''}
...
export function DLE(){window.dl(new Blob([JSON.stringify({log:S.log},null,2)],{type:'application/json'}),`DSGVO_${new Date().toISOString().slice(0,10)}.json`)}
export function DA(){S.xH=[];S.xD=[];S.xFn='';S.xBak=null;S.filtered=false;S.wH=[];S.wD=[];S.wI=false;S.wPv=null;S.calcCols=[];S.savedCases=[];S.savedIE=[];S.savedSW=[];S.macros=[];S.macSteps=[];S.macRec=false;S.pipelines=[];S.pipeSteps=[];S.selectedRows=new Set();S.hiddenCols=new Set();S.sortCol=-1;S.sortDir='asc';S.undoStack=[];S.redoStack=[];if(S.chart){S.chart.destroy();S.chart=null}$('x-empty').style.display='';$('status-bar').classList.remove('show');['x-exp','x-csv','x-add','x-acol','x-delcol','x-rencol','x-hidecol','x-selact','x-expsel','x-tbl','x-tabs','x-pdf','x-undoredo','x-quick-ops'].forEach(id=>{if($(id))$(id).style.display='none'});IDB.clear('autoSave').catch(()=>{});IDB.clear('workspaces').catch(()=>{});IDB.clear('templates').catch(()=>{});IDB.clear('hybrid').catch(()=>{});IDB.clear('docConfig').catch(()=>{});IDB.clear('docCenter').catch(()=>{});IDB.clear('database').catch(()=>{});window._wsCache=[];_tplCache=[];window._docxHeader=null;window._docxFooter='';window._dcTemplates=[];window._dcCategories=[...window.DC_DEFAULT_CATS];window._dcTriggers=[];window._dbEntries=[];window._dbSelected=new Set();$('x-sheet').style.display='none';window._wb=null;['fm-out','pv-out','fi-info','sr-out','dd-out','vl-out','cs-result','ie-result','calc-history','cs-saved','ie-saved','mac-current','mac-list','pp-steps','pp-saved'].forEach(id=>{if($(id))$(id).innerHTML=''});$('ch-wrap').style.display='none';$('rec-badge').style.display='none';$('w-tpl').value='';window.UP();['w-empty'].forEach(id=>$(id).style.display='');['w-dt','w-merge','w-pvbox','w-use'].forEach(id=>{if($(id))$(id).style.display='none'});['em-to','em-cc','em-subj','em-body','sem-subj'].forEach(id=>{if($(id))$(id).value=''});if($('sem-list'))$('sem-list').innerHTML='';if($('sem-btn'))$('sem-btn').disabled=true;$('del-cfm').style.display='none';if($('data-profile'))$('data-profile').classList.remove('show');if($('insights-panel'))$('insights-panel').classList.remove('show');if($('quick-actions'))$('quick-actions').classList.remove('show');if($('formula-bar'))$('formula-bar').classList.remove('show');if($('fs-toggle'))$('fs-toggle').classList.remove('show');if($('smart-suggest'))$('smart-suggest').classList.remove('show');window._activeCell=null;window._colFilters={};S._autoFitted=false;window._dismissed.clear();window._ssDataHash='';L('LÖSCHUNG','Art.17');toast('Gelöscht')}
