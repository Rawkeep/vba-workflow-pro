...
export function XDC(ci){if(ci<0||ci>=S.xH.length)return;pushUndo();const name=S.xH[ci];S.xH.splice(ci,1);S.xD.forEach(r=>r.splice(ci,1));S.hiddenCols.delete(ci);const nh=new Set();S.hiddenCols.forEach(h=>{nh.add(h>ci?h-1:h)});S.hiddenCols=nh;showX();XR();L('Spalte gelöscht',name);toast(`Spalte "${name}" gelöscht`)}
export function XRenC(ci){const old=S.xH[ci];const n=prompt(`Spalte "${old}" umbenennen:`,old);if(!n||n===old)return;pushUndo();S.xH[ci]=n;showX();XR();L('Spalte umbenannt',`${old}→${n}`);toast(`"${old}" → "${n}"`)}
export function XHideC(ci){S.hiddenCols.add(ci);XR();updateHiddenBadge();toast(`"${S.xH[ci]}" ausgeblendet`)}
export function XShowC(ci){S.hiddenCols.delete(ci);XR();updateHiddenBadge()}
export function XShowAllC(){S.hiddenCols.clear();XR();updateHiddenBadge();toast('Alle Spalten sichtbar')}
...
export function showHiddenCols(ev){const items=[...S.hiddenCols].map(ci=>`<div class="ctx-item" onclick="XShowC(${ci});hideCtx()">${S.xH[ci]}</div>`).join('');showCtxAt(ev,`<div class="ctx-item" style="color:var(--acc);font-weight:600;pointer-events:none">Ausgeblendete Spalten</div><div class="ctx-sep"></div>${items}<div class="ctx-sep"></div><div class="ctx-item" onclick="XShowAllC();hideCtx()">Alle einblenden</div>`)}
export function XMoveC(ci,dir){const ni=ci+dir;if(ni<0||ni>=S.xH.length)return;pushUndo();[S.xH[ci],S.xH[ni]]=[S.xH[ni],S.xH[ci]];S.xD.forEach(r=>{[r[ci],r[ni]]=[r[ni],r[ci]]});XR();toast('Spalte verschoben')}
export function showColPicker(ev,action){
  if(action==='delete'){
    ...
    S.xH.forEach((h,i)=>{
      html+=`<div class="ctx-item" style="padding:5px 14px" onclick="event.stopPropagation();this.querySelector('input').checked=!this.querySelector('input').checked;_updateDelCount()"><input type="checkbox" data-ci="${i}" style="accent-color:var(--red);pointer-events:none;margin-right:6px">${H(h)}</div>`;
    });
  ...
  const items=S.xH.map((h,i)=>{if(action==='hide'&&S.hiddenCols.has(i))return'';let onclick='';if(action==='rename')onclick=`XRenC(${i})`;else if(action==='hide')onclick=`XHideC(${i})`;return`<div class="ctx-item" onclick="${onclick};hideCtx()">${H(h)}</div>`}).join('');
  const label=action==='rename'?'Spalte umbenennen':'Spalte ausblenden';
  showCtxAt(ev,`<div class="ctx-item" style="color:var(--acc);font-weight:600;pointer-events:none">${label}</div><div class="ctx-sep"></div>${items}`);
}
...
export function _deleteCheckedCols(){
  ...
  S.hiddenCols=nh;
  showX();XR();hideCtx();
  L('Spalten gelöscht',indices.length+'');
  toast(indices.length+' Spalten gelöscht');
}
...
export function colStats(ci){
  ...
  let html=`<div class="ctx-item" style="color:var(--acc);font-weight:600;pointer-events:none">📊 ${H(S.xH[ci])}</div><div class="ctx-sep"></div>`;
  ...
  if(nums.length){
    ...
    top.forEach(([v,c])=>{html+=`<div class="ctx-item" style="pointer-events:none;color:var(--tx2)">${H(v)} <span style="color:var(--acc)">(${c}×)</span></div>`});
  }
  return html;
}
...
export function initColStatsOverride() {
  ...
  window.ctxHeader=function(ev,ci){
    ...
    showCtxAt(ev,`<div class="ctx-item" style="color:var(--acc);font-weight:600;pointer-events:none">${H(h)}</div><div class="ctx-sep"></div><div class="ctx-item" onclick="sortByClick(${ci});hideCtx()">Sortieren A→Z / Z→A</div><div class="ctx-item" onclick="XRenC(${ci});hideCtx()">✏ Umbenennen</div><div class="ctx-item" onclick="XHideC(${ci});hideCtx()">👁 Ausblenden</div><div class="ctx-item" onclick="XMoveC(${ci},-1);hideCtx()">← Nach links</div><div class="ctx-item" onclick="XMoveC(${ci},1);hideCtx()">→ Nach rechts</div><div class="ctx-sep"></div>${stats}<div class="ctx-sep"></div><div class="ctx-item danger" onclick="XDC(${ci});hideCtx()">✕ Spalte löschen</div>`);
  };
}
