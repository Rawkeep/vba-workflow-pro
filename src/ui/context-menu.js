...
export function ctxCell(ev,ri,ci){ev.preventDefault();ev.stopPropagation();const val=S.xD[ri][ci];const cmt=window.getComment?window.getComment(ri,ci):'';showCtxAt(ev,`<div class="ctx-item" onclick="CE(${ri},${ci});hideCtx()">✏ Bearbeiten</div><div class="ctx-item" onclick="navigator.clipboard.writeText('${window.A(String(val??''))}');toast('📋');hideCtx()">📋 Kopieren</div><div class="ctx-item" onclick="pasteToCell(${ri},${ci});hideCtx()">📥 Einfügen</div><div class="ctx-sep"></div><div class="ctx-item" onclick="addComment(${ri},${ci});hideCtx()">💬 ${cmt?'Kommentar bearbeiten':'Kommentar hinzufügen'}</div><div class="ctx-item" onclick="fillDown(${ri},${ci});hideCtx()">↓ Nach unten füllen</div><div class="ctx-item" onclick="clearCell(${ri},${ci});hideCtx()">Zelle leeren</div><div class="ctx-sep"></div><div class="ctx-item" onclick="insertRowAbove(${ri});hideCtx()">+ Zeile darüber</div><div class="ctx-item" onclick="insertRowBelow(${ri});hideCtx()">+ Zeile darunter</div><div class="ctx-item danger" onclick="XDR(${ri});hideCtx()">✕ Zeile löschen</div>`)}
