...
export function _lsUsage(){return window.IDB.usage()}
export async function autoSave(){
  ...
  try{
    ...
    await window.IDB.put('autoSave','current',data);
    const b=$('autosave-badge');b.classList.add('show');setTimeout(()=>b.classList.remove('show'),1500);
  }catch(e){window.toast('⚠ AutoSave fehlgeschlagen');window._appLog('AutoSave: '+e.message)}
}
export async function autoRestore(){
  try{
    // Migrate from localStorage if exists
    const legacy=localStorage.getItem('vbaBeastAutoSave');
    if(legacy){const ld=JSON.parse(legacy);await window.IDB.put('autoSave','current',ld);localStorage.removeItem('vbaBeastAutoSave')}
    const data=await window.IDB.get('autoSave','current');
    if(!data||!data.xH||!data.xH.length)return false;
    if(Date.now()-data.ts>86400000){await window.IDB.del('autoSave','current');return false}
    ...
    window.N('excel');window.showX();window.XR();
    window.toast('♻ Letzte Sitzung wiederhergestellt');window.L('Restore',S.xFn);
    return true;
  }catch(e){window._appLog('autoRestore: '+e.message);return false}
}
...
export function qSearchClose(){$('qsearch').classList.remove('show');$('qs-input').value='';_qsMatches=[];_qsIdx=-1;$('qs-info').textContent='';window.XR()}
export function qSearch(val){
  _qsMatches=[];_qsIdx=-1;
  if(!val.trim()||!S.xH.length){$('qs-info').textContent='';window.XR();return}
