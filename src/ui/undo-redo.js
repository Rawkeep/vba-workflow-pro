...
export function UNDO(){if(!S.undoStack.length)return;S.redoStack.push(_snapState());const s=S.undoStack.pop();S.xH=s.xH;S.xD=s.xD;showX();XR();updateUndoBtn();toast('↩ Rückgängig')}
export function REDO(){if(!S.redoStack.length)return;S.undoStack.push(_snapState());const s=S.redoStack.pop();S.xH=s.xH;S.xD=s.xD;showX();XR();updateUndoBtn();toast('↪ Wiederholt')}
...
export function wrapMutatorsWithUndo() {
  const _origXDR=XDR;XDR=function(i){pushUndo();_origXDR(i)};
  const _origCC=CC;CC=function(r,c,el){pushUndo();_origCC(r,c,el)};
  const _origXAR=XAR;XAR=function(){pushUndo();_origXAR()};
  const _origXAC=XAC;XAC=function(){pushUndo();_origXAC()};
}
