...
export const _colOptIds=['cc-col','so-col','fi-col','tf-col','dd-col','vl-scol','vl-rcol','ch-label','ch-data','pv-g','pv-a','cs-src','cs-tgt','ie-tgt','vd-col','sw-tgt'];
export function showX(){$('x-empty').style.display='none';for(let i=0;i<_showXIds.length;i++){const el=$(_showXIds[i]);if(el)el.style.display=''}$('status-bar').classList.add('show');$('x-info').textContent=`${S.xFn} — ${S.xD.length}×${S.xH.length}`;const o=colOpts();for(let i=0;i<_colOptIds.length;i++){const el=$(_colOptIds[i]);if(el)el.innerHTML=o}$('sr-col').innerHTML='<option value="__ALL__">Alle</option>'+S.xH.map(h=>`<option value="${A(h)}">${H(h)}</option>`).join('');document.querySelectorAll('.ie-col,.sw-col1,.sw-col2').forEach(el=>el.innerHTML=o);$('w-use').style.display='';updateStatusBar();updateHiddenBadge();checkSEM();renderCalcH();renderSavedCases();renderSavedIE();renderMacList();renderSavedPipes();renderSavedSW()}
export const _VS_ROW_H=28;export const _VS_MAX=200; // virtual scroll: max rows rendered at once
export let _vsStart=0;
export function _vsRenderRow(ri,ci,cA,co,cv,cn){
  const row=S.xD[ri];if(!row)return'';
  const sel=S.selectedRows&&S.selectedRows.has(ri);
  const cells=row.map((c,ci2)=>{if(S.hiddenCols&&S.hiddenCols.has(ci2))return'';let cls='ed';
    if(cA&&ci2===ci){const v=parseFloat(c),s=String(c);let m=false;if(co==='enthält')m=s.toLowerCase().includes(cv.toLowerCase());else if(co==='≠')m=s!==cv;else if(!isNaN(v)&&!isNaN(cn)){if(co==='>')m=v>cn;else if(co==='<')m=v<cn;else if(co==='>=')m=v>=cn;else if(co==='<=')m=v<=cn;else if(co==='=')m=v===cn}else if(co==='=')m=s===cv;if(m)cls+=' hl'}
    return`<td class="${cls}" onclick="CE(${ri},${ci2})" oncontextmenu="ctxCell(event,${ri},${ci2})" id="c${ri}-${ci2}">${H(String(c??''))}</td>`}).join('');
...
export function XR(){const cc=$('cc-col').value,co=$('cc-op').value,cv=$('cc-val').value,ci=S.xH.indexOf(cc),cn=parseFloat(cv),cA=cc&&cv!=='';$('cc-st').innerHTML=cA?'<span class="tg tg-r">✓</span>':'';
_typeCache=null;_qualCache=null; // invalidate caches on data change
...
for(let ri=renderStart;ri<renderEnd;ri++)rows+=_vsRenderRow(ri,ci,cA,co,cv,cn);
...
if(useVS){const tw=$('x-tw');if(!tw._vsEvt){tw._vsEvt=true;tw.addEventListener('scroll',_debounce(function(){const st=tw.scrollTop;const newStart=Math.max(0,Math.floor(st/_VS_ROW_H)-20);if(Math.abs(newStart-_vsStart)>10){_vsStart=newStart;XR()}},60),{passive:true})}}
$('x-info').textContent=`${S.xFn} — ${S.xD.length}×${S.xH.length}${S.filtered?' ⚡':''}${useVS?' (virtuell)':''}`;updateStatusBar()}
