...
// ══════ EXISTING OPS ══════
export function FM(){const raw=$('fm-in').value.trim().toUpperCase();const sif=raw.match(/^(?:SUMIF|SUMMEWENN)\((.+),(.+),(.+)\)$/);if(sif){const[,cn,cd,vn]=sif;const ci=S.xH.findIndex(h=>h.toUpperCase()===cn.trim()),vi=S.xH.findIndex(h=>h.toUpperCase()===vn.trim());if(ci===-1||vi===-1){$('fm-out').innerHTML='<span class="tg tg-r">?</span>';return}let s=0;S.xD.forEach(r=>{if(String(r[ci]).toUpperCase()===cd.trim()){const v=parseFloat(r[vi]);if(!isNaN(v))s+=v}});$('fm-out').innerHTML=`<span class="fr">= ${s.toFixed(2)}</span>`;return}const m=raw.match(/^(SUM|SUMME|AVG|AVERAGE|MITTELWERT|COUNT|ANZAHL|COUNTIF|ANZAHLWENN|MIN|MAX|MEDIAN|STDEV|STABW)\((.+)\)$/);if(!m){$('fm-out').innerHTML='<span class="tg tg-r">?</span>';return}let[,fn,cn]=m;const _fmAlias={SUMME:'SUM',MITTELWERT:'AVG',ANZAHL:'COUNT',ANZAHLWENN:'COUNTIF',STABW:'STDEV'};if(_fmAlias[fn])fn=_fmAlias[fn];const ci=S.xH.findIndex(h=>h.toUpperCase()===cn.trim().toUpperCase());if(ci===-1){$('fm-out').innerHTML='<span class="tg tg-r">?</span>';return}const vs=S.xD.map(r=>parseFloat(r[ci])).filter(v=>!isNaN(v));if(!vs.length){$('fm-out').innerHTML='<span class="tg tg-r">∅</span>';return}let res;switch(fn){case'SUM':res=vs.reduce((a,b)=>a+b,0).toFixed(2);break;case'AVG':case'AVERAGE':res=(vs.reduce((a,b)=>a+b,0)/vs.length).toFixed(2);break;case'COUNT':res=vs.length;break;case'COUNTIF':res=S.xD.filter(r=>r[ci]!==''&&r[ci]!=null).length;break;case'MIN':res=Math.min(...vs).toFixed(2);break;case'MAX':res=Math.max(...vs).toFixed(2);break;case'MEDIAN':vs.sort((a,b)=>a-b);const mid=Math.floor(vs.length/2);res=vs.length%2?vs[mid].toFixed(2):((vs[mid-1]+vs[mid])/2).toFixed(2);break;case'STDEV':const avg=vs.reduce((a,b)=>a+b,0)/vs.length;res=Math.sqrt(vs.reduce((s,v)=>s+Math.pow(v-avg,2),0)/vs.length).toFixed(4);break}$('fm-out').innerHTML=`<span class="fr">= ${res}</span>`}
...
export function SO(){const col=$('so-col').value,dir=$('so-dir').value;if(!col)return;pushUndo();const ci=S.xH.indexOf(col);S.xD.sort((a,b)=>{const na=parseFloat(a[ci]),nb=parseFloat(b[ci]);if(!isNaN(na)&&!isNaN(nb))return dir==='asc'?na-nb:nb-na;return dir==='asc'?String(a[ci]).localeCompare(String(b[ci])):String(b[ci]).localeCompare(String(a[ci]))});XR();L('Sort',`${col}`);toast('✓');macLog('sort',{col,dir})}
export function FI(){const col=$('fi-col').value,op=$('fi-op').value,val=$('fi-val').value;if(!col)return;const ci=S.xH.indexOf(col);if(!S.xBak)S.xBak=S.xD.map(r=>[...r]);S.xD=S.xBak.filter(r=>{const c=String(r[ci]??''),n=parseFloat(c),nv=parseFloat(val);switch(op){case'enthält':return c.toLowerCase().includes(val.toLowerCase());case'beginnt mit':return c.toLowerCase().startsWith(val.toLowerCase());case'endet mit':return c.toLowerCase().endsWith(val.toLowerCase());case'=':return c===val;case'≠':return c!==val;case'>':return!isNaN(n)&&n>nv;case'<':return!isNaN(n)&&n<nv;case'leer':return!c.trim();case'nicht leer':return!!c.trim()}return true});S.filtered=true;XR();$('fi-info').textContent=`${S.xD.length}/${S.xBak.length}`;L('Filter',`${col} ${op}`);toast('✓');macLog('filter',{col,op,val})}
export function FIR(){if(S.xBak){S.xD=S.xBak;S.xBak=null;S.filtered=false;XR();$('fi-info').textContent='';toast('✓');macLog('filterReset',{})}}
export function SR(){const f=$('sr-find').value,r=$('sr-rep').value,col=$('sr-col').value;if(!f)return;pushUndo();let c=0;S.xD.forEach(row=>row.forEach((v,ci)=>{if(col!=='__ALL__'&&S.xH[ci]!==col)return;const s=String(v);if(s.includes(f)){row[ci]=s.split(f).join(r);c++}}));XR();$('sr-out').textContent=c+' ersetzt';toast(c+' ✓');macLog('sr',{find:f,rep:r,col})}
export function TF(){const col=$('tf-col').value,fn=$('tf-fn').value,n=parseInt($('tf-n').value)||5;if(!col)return;pushUndo();const ci=S.xH.indexOf(col);S.xD.forEach(r=>{let s=String(r[ci]??'');switch(fn){case'UPPER':r[ci]=s.toUpperCase();break;case'LOWER':r[ci]=s.toLowerCase();break;case'TRIM':r[ci]=s.trim().replace(/\s+/g,' ');break;case'PROPER':r[ci]=s.toLowerCase().replace(/(^|\s)\S/g,c=>c.toUpperCase());break;case'LEN':r[ci]=s.length;break;case'LEFT(n)':r[ci]=s.substring(0,n);break;case'RIGHT(n)':r[ci]=s.substring(s.length-n);break;case'REVERSE':r[ci]=s.split('').reverse().join('');break}});XR();toast('✓');macLog('textfn',{col,fn})}
export function DD(){const col=$('dd-col').value;if(!col)return;pushUndo();const ci=S.xH.indexOf(col),b=S.xD.length,seen=new Set();S.xD=S.xD.filter(r=>{const v=String(r[ci]);if(seen.has(v))return false;seen.add(v);return true});XR();$('dd-out').textContent=`${b-S.xD.length} entf.`;toast('✓');macLog('dedup',{col})}
...
export function VD_RUN(){
  ...
  if(!col){toast('Spalte wählen');return}pushUndo();
  ...
  S.xD.forEach(r=>{
    ...
    switch(type){
      ...
      case'regex':try{pass=new RegExp(p1).test(v)}catch{pass=false};break;
