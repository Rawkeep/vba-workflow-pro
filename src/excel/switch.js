...
export function swAddNew(){const n=prompt('Neue Spalte:');if(!n)return;if(S.xH.includes(n)){toast('⚠ Spalte "'+n+'" existiert bereits');$('sw-tgt').value=n;return}S.xH.push(n);S.xD.forEach(r=>r.push(''));window.showX();window.XR();$('sw-tgt').value=n}
...
export function SW_RUN(){
  ...
  const ti=S.xH.indexOf(tgt);if(ti===-1)return;window.pushUndo();
...
export function runSavedSW(idx){const s=S.savedSW[idx];const ti=S.xH.indexOf(s.tgt);if(ti===-1)return;window.pushUndo();let n=0;
