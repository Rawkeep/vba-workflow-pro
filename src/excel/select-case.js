...
export function csAddNew(){const n=prompt('Neue Spalte:');if(!n)return;if(S.xH.includes(n)){toast('⚠ Spalte "'+n+'" existiert bereits');$('cs-tgt').value=n;return}S.xH.push(n);S.xD.forEach(r=>r.push(''));window.showX();window.XR();$('cs-tgt').value=n}
export function CS_RUN(){
  ...
  window.pushUndo();const cases=[];document.querySelectorAll('#cs-rows .case-row').forEach(row=>{
  ...
  window.XR();$('cs-result').innerHTML=`<span class="tg tg-g">${count} Zeilen</span>`;
