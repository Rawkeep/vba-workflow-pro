...
export function CALC(){const name=$('calc-name').value.trim(),f=$('calc-formula').value.trim();if(!name||!f)return;pushUndo();S.xH.push(name);S.xD.forEach(r=>r.push(evalCalc(f,r)));S.calcCols.push({name,f});showX();XR();renderCalcH();L('Calc',`${name}=${f}`);toast('"'+name+'" ✓');macLog('calc',{name,f})}
...
export function reCalc(i){const c=S.calcCols[i],ci=S.xH.indexOf(c.name);if(ci===-1)return;S.xD.forEach(r=>r[ci]=evalCalc(c.f,r));XR();toast('🔄 ✓')}
