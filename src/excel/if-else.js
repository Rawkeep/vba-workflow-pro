...
export function ieAddNew(){const n=prompt('Neue Spalte:');if(!n)return;if(S.xH.includes(n)){toast('⚠ Spalte "'+n+'" existiert bereits');$('ie-tgt').value=n;return}S.xH.push(n);S.xD.forEach(r=>r.push(''));window.showX();window.XR();$('ie-tgt').value=n}
...
export function IE_RUN(){
  ...
  window.pushUndo();const blocks=[...document.querySelectorAll('#ie-chain .ie-block')];let count=0;
  S.xD.forEach(row=>{
    let matched=false;
...
export function runSavedIE(idx){const ie=S.savedIE[idx];const ti=S.xH.indexOf(ie.tgt);if(ti===-1)return;window.pushUndo();let n=0;
