...
export function toast(m,type='success'){const t=$('toast');t.className='';t.offsetHeight;/* reflow */t.textContent=m;t.classList.add('show','toast-'+type);clearTimeout(t._tid);t._tid=setTimeout(()=>t.classList.remove('show'),type==='error'?4000:2200)}
export const _hDiv=document.createElement('div');export function H(s){_hDiv.textContent=s;return _hDiv.innerHTML}
export function confirmAction(msg,onYes){const o=document.createElement('div');o.className='confirm-overlay';o.innerHTML=`<div class="confirm-dialog"><p>${H(msg)}</p><div style="display:flex;gap:8px;justify-content:flex-end"><button class="b bo bs" data-a="no">Abbrechen</button><button class="b bd bs" data-a="yes">Ja, löschen</button></div></div>`;document.body.appendChild(o);o.querySelector('[data-a="no"]').onclick=()=>o.remove();o.querySelector('[data-a="yes"]').onclick=()=>{o.remove();onYes()}}
